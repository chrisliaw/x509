defmodule X509.PrivateKeyTest do
  ...
  describe "EC" do
    test "new" do
      ...
      assert match?(ec_private_key(), new_ec(:ed25519))
      assert match?(ec_private_key(), new_ec(oid(:"id-Ed25519")))
      assert match?(ec_private_key(), new_ec(:ed448))
      assert match?(ec_private_key(), new_ec(oid(:"id-Ed448")))
    end

    test "wrap and unwrap: prime256v1", context do
    ...
    for curve <- ["prime256v1", "ed25519", "ed448"] do
      ...
      test "PEM decode and encode: #{@curve}", context do
        pem = File.read!("test/data/#{@curve}.pem")
        assert match?({:ok, ec_private_key()}, from_pem(pem))

        assert context.ec_key == context.ec_key |> to_pem() |> from_pem!()

        pem_des3 = File.read!("test/data/#{@curve}_des3.pem")
        assert match?({:ok, ec_private_key()}, from_pem(pem_des3, password: "secret"))

        pem_aes = File.read!("test/data/#{@curve}_aes.pem")
        assert match?({:ok, ec_private_key()}, from_pem(pem_aes, password: "secret"))

        res = to_pem(context.ec_key, password: "secret")
        IO.inspect(res)

      end

      test "PKCS8 PEM decode and encode: #{@curve}", context do
        pem = File.read!("test/data/#{@curve}_pkcs8.pem")
        assert match?({:ok, ec_private_key()}, from_pem(pem))
      ...
      test "PKCS8 DER decode and encode: #{@curve}" do
        der = File.read!("test/data/#{@curve}_pkcs8.der")
        assert match?(ec_private_key(), from_der!(der))
        assert der == der |> from_der!() |> to_der(wrap: true)
      end
    end

    test "DER decode and encode: prime256v1}" do
